...
export function initShaders(gl:WebGLRenderingContext|WebGL2RenderingContext, vertexShaderId:string, fragmentShaderId:string): WebGLProgram {
...
export function initFileShaders(gl:WebGLRenderingContext|WebGL2RenderingContext, vShaderName:string, fShaderName:string):WebGLProgram {
    function getShader(gl:WebGLRenderingContext|WebGL2RenderingContext, shaderName:string, type:GLenum) :WebGLShader {
...
export function translate( x:number | vec3, y:number=0, z:number=0 ): mat4
{
...
export function scalem( x:number | vec3, y:number=1, z:number=1 )
{
    if ( Array.isArray(x) && x.length == 3 ) {
        z = x[2];
        y = x[1];
